...
module ExprTypes =
    ...
    type LeafOp = Expr.LeafOp
    type UnaryOp = Expr.UnaryOp
    type BinaryOp = Expr.BinaryOp
    type NaryOp = Expr.NaryOp
    type IOp = Expr.IOp
    type Expr = Expr.Expr
    type MultiChannelOp = Expr.MultiChannelOp
    type Channel = Expr.Channel
    type UnaryHeldOp = Expr.UnaryHeldOp
    type SequenceArgSlice = Expr.SequenceArgSlice
    type PreviousChannel = Expr.PreviousChannel
    type LoopInput = Expr.LoopInput
    type LoopValue = Expr.LoopValue
    type LoopSpec = Expr.LoopSpec
