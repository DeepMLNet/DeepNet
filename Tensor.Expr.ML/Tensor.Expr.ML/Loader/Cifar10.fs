...
type CifarRaw = {
    ...
    member this.Transfer dev =
        {TrnData = this.TrnData |> Tensor.transfer dev
         TrnLbls = this.TrnLbls |> Tensor.transfer dev
         TstData = this.TstData |> Tensor.transfer dev
         TstLbls = this.TstLbls |> Tensor.transfer dev}
...
module Cifar10 =
    ...
    let load directory valRatio =
        ...
        let orgTrn = Dataset<InpTgtSmpl<single>> [trnData; raw.TrnLbls]

        let trn, vali =
            match orgTrn |> Dataset.partition [1. - valRatio; valRatio] with
            | [trn; vali] -> trn, vali
            | _ -> failwith "impossible"
        let tst = Dataset<InpTgtSmpl<single>> [tstData; raw.TstLbls]
