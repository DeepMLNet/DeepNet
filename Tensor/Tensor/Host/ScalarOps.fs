...
type internal ScalarOps =
    ...
    static member inline ApplyUnaryOp (scalarOp: int64[] -> 'T1 -> 'T, 
        ...
        let inline loops (dim0Fixed: bool) (dim0Pos: int) =
            ...
            while trgtPosIter.Active do
                ...
                if nd = 0 then
                    trgtSpan.[trgtPosIter.Addr] <- scalarOp [||] src1Span.[src1PosIter.Addr] 
                elif isIndexed then
                    for d in 0 .. nd - 1 do
                        pos64.[d] <- int64 trgtPosIter.Pos.[d]
                    for i in 0 .. shape.[nd-1] - 1 do
                        trgtSpan.[trgtAddr] <- scalarOp pos64 src1Span.[src1Addr] 
                        trgtAddr <- trgtAddr + trgt.FastLayout.Stride.[nd-1]
                        src1Addr <- src1Addr + src1.FastLayout.Stride.[nd-1]
                        pos64.[nd-1] <- pos64.[nd-1] + 1L
                else
                    for i in 0 .. shape.[nd-1] - 1 do
                        trgtSpan.[trgtAddr] <- scalarOp [||] src1Span.[src1Addr] 
                        trgtAddr <- trgtAddr + trgt.FastLayout.Stride.[nd-1]
                        src1Addr <- src1Addr + src1.FastLayout.Stride.[nd-1]
                trgtPosIter.MoveNext()
    ...
    static member inline ApplyUnaryMethod (scalarOp: int64[] -> 'T1 -> unit, 
        ...
        let inline loops (dim0Fixed: bool) (dim0Pos: int) =
            ...
            while src1PosIter.Active do
                ...
                if nd = 0 then
                    scalarOp [||] src1Span.[src1PosIter.Addr] 
                elif isIndexed then
                    for d in 0 .. nd - 1 do
                        pos64.[d] <- int64 src1PosIter.Pos.[d]
                    for i in 0 .. shape.[nd-1] - 1 do
                        scalarOp pos64 src1Span.[src1Addr] 
                        src1Addr <- src1Addr + src1.FastLayout.Stride.[nd-1]
                        pos64.[nd-1] <- pos64.[nd-1] + 1L
                else
                    for i in 0 .. shape.[nd-1] - 1 do
                        scalarOp [||] src1Span.[src1Addr] 
                        src1Addr <- src1Addr + src1.FastLayout.Stride.[nd-1]
                src1PosIter.MoveNext()
    ...
    static member inline ApplyBinaryOp (scalarOp: int64[] -> 'T1 -> 'T2 -> 'T, 
        ...
        let inline loops (dim0Fixed: bool) (dim0Pos: int) =
            ...
            while trgtPosIter.Active do
                ...
                if nd = 0 then
                    trgtSpan.[trgtPosIter.Addr] <- 
                        scalarOp [||] src1Span.[src1PosIter.Addr] src2Span.[src2PosIter.Addr]
                elif isIndexed then
                    for d in 0 .. nd - 1 do
                        pos64.[d] <- int64 trgtPosIter.Pos.[d]
                    for i in 0 .. shape.[nd-1] - 1 do
                        trgtSpan.[trgtAddr] <- scalarOp pos64 src1Span.[src1Addr] src2Span.[src2Addr]
                        trgtAddr <- trgtAddr + trgt.FastLayout.Stride.[nd-1]
                        src1Addr <- src1Addr + src1.FastLayout.Stride.[nd-1]
                    ...
                    for i in 0 .. shape.[nd-1] - 1 do
                        trgtSpan.[trgtAddr] <- scalarOp [||] src1Span.[src1Addr] src2Span.[src2Addr]
                        trgtAddr <- trgtAddr + trgt.FastLayout.Stride.[nd-1]
                        src1Addr <- src1Addr + src1.FastLayout.Stride.[nd-1]
                        src2Addr <- src2Addr + src2.FastLayout.Stride.[nd-1]
                trgtPosIter.MoveNext()
    ...
    static member inline ApplyTernaryOp (scalarOp: int64[] -> 'T1 -> 'T2 -> 'T3 -> 'T, 
        ...
        let inline loops (dim0Fixed: bool) (dim0Pos: int) =
            ...
            while trgtPosIter.Active do
                ...
                if nd = 0 then
                    trgtSpan.[trgtPosIter.Addr] <- 
                        scalarOp [||] src1Span.[src1PosIter.Addr] src2Span.[src2PosIter.Addr] src3Span.[src3PosIter.Addr]
                elif isIndexed then
                    for d in 0 .. nd - 1 do
                        pos64.[d] <- int64 trgtPosIter.Pos.[d]
                    for i in 0 .. shape.[nd-1] - 1 do
                        trgtSpan.[trgtAddr] <- scalarOp pos64 src1Span.[src1Addr] src2Span.[src2Addr] src3Span.[src3Addr]
                        trgtAddr <- trgtAddr + trgt.FastLayout.Stride.[nd-1]
                        src1Addr <- src1Addr + src1.FastLayout.Stride.[nd-1]
                        src2Addr <- src2Addr + src2.FastLayout.Stride.[nd-1]
                    ...
                    for i in 0 .. shape.[nd-1] - 1 do
                        trgtSpan.[trgtAddr] <- scalarOp [||] src1Span.[src1Addr] src2Span.[src2Addr] src3Span.[src3Addr]
                        trgtAddr <- trgtAddr + trgt.FastLayout.Stride.[nd-1]
                        src1Addr <- src1Addr + src1.FastLayout.Stride.[nd-1]
                        src2Addr <- src2Addr + src2.FastLayout.Stride.[nd-1]
                        src3Addr <- src3Addr + src3.FastLayout.Stride.[nd-1]
    ...
    static member inline ApplyNAryOp (scalarOp: int64[] -> 'T[] -> 'T, 
            ...
            if isIndexed then
                ...
                for pos in 0 .. shape.[nd-1] - 1 do
                    for s in 0 .. nSrcs-1 do
                        srcVals.[s] <- srcs.[s].Span.[srcsAddr.[s]]
                    trgtSpan.[trgtAddr] <- scalarOp pos64 srcVals
                    trgtAddr <- trgtAddr + trgt.FastLayout.Stride.[nd-1]
                    for s in 0 .. nSrcs-1 do
                ...
                for pos in 0 .. shape.[nd-1] - 1 do
                    for s in 0 .. nSrcs-1 do
                        srcVals.[s] <- srcs.[s].Span.[srcsAddr.[s]]
                    trgtSpan.[trgtAddr] <- scalarOp [||] srcVals
                    trgtAddr <- trgtAddr + trgt.FastLayout.Stride.[nd-1]
                    for s in 0 .. nSrcs-1 do
                        srcsAddr.[s] <- srcsAddr.[s] + srcs.[s].FastLayout.Stride.[nd-1]
        ...
        let inline scalarInnerLoop (trgtAddr: int) (srcsAddr: int[]) =
            ...
            let srcsValue = 
                (srcs, srcsAddr) 
                ||> Array.map2 (fun (src: DataAndLayout<'T>) addr -> src.Span.[addr])
            trgtSpan.[trgtAddr] <- scalarOp [||] srcsValue
    ...
    static member inline ApplyAxisFold (foldOp: int64[] -> 'TS -> 'T1 -> 'TS, 
        ...
        let inline loops (dim0Fixed: bool) (dim0Pos: int) =
            ...
            while trgtPosIter.Active do
                ...
                if nd = 0 then
                    trgtSpan.[trgtPosIter.Addr] <- foldOp [||] state src1Span.[src1Addr] |> extractOp
                elif isIndexed then
                    for d in 0 .. nd-2 do
                        pos64.[d] <- int64 trgtPosIter.Pos.[d]
                    pos64.[nd-1] <- 0L
                    for i in 0 .. shape.[nd-1] - 1 do
                        state <- foldOp pos64 state src1Span.[src1Addr]
                        src1Addr <- src1Addr + src1.FastLayout.Stride.[nd-1]
                        pos64.[nd-1] <- pos64.[nd-1] + 1L
                    trgtSpan.[trgtPosIter.Addr] <- extractOp state
                else
                    for i in 0 .. shape.[nd-1] - 1 do
                        state <- foldOp [||] state src1Span.[src1Addr]
                        src1Addr <- src1Addr + src1.FastLayout.Stride.[nd-1]
                    trgtSpan.[trgtPosIter.Addr] <- extractOp state
                trgtPosIter.MoveNext()
                src1PosIter.MoveNext()
    ...
    static member inline ApplyAxisFoldInitialTensor (foldOp: int64[] -> 'TS -> 'T1 -> 'TS, 
                                                     extractOp: 'TS -> 'T, 
                                                     trgt: DataAndLayout<'T>, src1: DataAndLayout<'T1>, 
                                                     initial: DataAndLayout<'TS>,
                                                     isIndexed: bool, useThreads: bool) =        
        let nd = src1.FastLayout.NDims
        let shape = src1.FastLayout.Shape
        ...
        let inline loops (dim0Fixed: bool) (dim0Pos: int) =
            ...
            let mutable trgtPosIter = 
                PosIter32 (trgt.FastLayout, startPos.[0 .. nd-2], fromDim=fromDim, toDim=nd-2)
            let initialPosIter =
                Some (ref (PosIter32 (initial.FastLayout, startPos.[0 .. nd-2], fromDim=fromDim, toDim=nd-2)))
            let mutable src1PosIter = 
                PosIter32 (src1.FastLayout, startPos, fromDim=fromDim, toDim=nd-2)
            let pos64 = Array.zeroCreate nd
            while trgtPosIter.Active do
                let mutable src1Addr = src1PosIter.Addr
                let mutable state = initial.Span.[initialPosIter.Value.contents.Addr]
                if nd = 0 then
                    trgtSpan.[trgtPosIter.Addr] <- foldOp [||] state src1Span.[src1Addr] |> extractOp
                elif isIndexed then
                    for d in 0 .. nd-2 do
                    ...
                    trgtSpan.[trgtPosIter.Addr] <- extractOp state
                trgtPosIter.MoveNext()
                initialPosIter.Value.contents.MoveNext()
                src1PosIter.MoveNext()
    ...
    static member IfThenElse (trgt: DataAndLayout<'T>, cond: DataAndLayout<bool>, 
        ...
        let p = ScalarPrimitives.For<'T, 'T>()
        let inline op _pos c t f = p.IfThenElse c t f
    ...
    static member Gather (trgt: DataAndLayout<'T>, srcIndices: DataAndLayout<int64> option [],
        ...
        let inline op (trgtIdx: int64[]) = 
            let srcIdx = Array.init src.FastLayout.NDims (fun dim ->
                ...
                | Some i -> i.Span.[i.FastLayout.Addr trgtIdx]
                | None -> trgtIdx.[dim])
            src.Span.[src.FastLayout.Addr srcIdx]                                      
        ScalarOps.ApplyNoaryOp (op, trgt, isIndexed=true, useThreads=true)         
    ...
    static member Scatter (trgt: DataAndLayout<'T>, trgtIndices: DataAndLayout<int64> option [],
        ...
        let inline op (srcIdx: int64[]) (srcVal: 'T) = 
            let trgtIdx = Array.init trgt.FastLayout.NDims (fun dim ->
                ...
                | Some i -> i.Span.[i.FastLayout.Addr srcIdx]
                | None -> srcIdx.[dim])
            let prvVal = trgt.Span.[trgt.FastLayout.Addr trgtIdx]
            trgt.Span.[trgt.FastLayout.Addr trgtIdx] <- p.Add prvVal srcVal
    ...
    static member CountTrueLastAxis (trgt: DataAndLayout<int64>, src1: DataAndLayout<bool>) =
        let inline op (_srcIdx: int64[]) (res: int64) (v: bool) = if v then res+1L else res
        ScalarOps.ApplyAxisFold (op, id, trgt, src1, initial=0L, isIndexed=false, useThreads=true)     

    static member SumLastAxis (trgt: DataAndLayout<'T>, src1: DataAndLayout<'T>) =
        let p = ScalarPrimitives.For<'T, 'T>()
        let inline op (_srcIdx: int64[]) (res: 'T) (v: 'T) = p.Add res v
    ...
    static member ProductLastAxis (trgt: DataAndLayout<'T>, src1: DataAndLayout<'T>) =
        let p = ScalarPrimitives.For<'T, 'T>()
        let inline op (_srcIdx: int64[]) (res: 'T) (v: 'T) = p.Multiply res v
    ...
    static member MaxLastAxis (trgt: DataAndLayout<'T>, src1: DataAndLayout<'T>) =
        let p = ScalarPrimitives.For<'T, 'T>()
        let inline op (_srcIdx: int64[]) (res: 'T) (v: 'T) = if p.Greater res v then res else v
    ...
    static member MinLastAxis (trgt: DataAndLayout<'T>, src1: DataAndLayout<'T>) =
        let p = ScalarPrimitives.For<'T, 'T>()
        let inline op (_srcIdx: int64[]) (res: 'T) (v: 'T) = if p.Less res v then res else v
    ...
    static member AllLastAxis (trgt: DataAndLayout<bool>, src1: DataAndLayout<bool>) =
        let inline op (_srcIdx: int64[]) (res: bool) (v: bool) = res && v
    ...
    static member ArgMaxLastAxis (trgt: DataAndLayout<int64>, src1: DataAndLayout<'T>) =
        ...
        ScalarOps.ApplyAxisFold (op, fst, trgt, src1, initial=(NotFound, minValue<'T>), 
    ...
    static member ArgMinLastAxis (trgt: DataAndLayout<int64>, src1: DataAndLayout<'T>) =
        ...
        ScalarOps.ApplyAxisFold (op, fst, trgt, src1, initial=(NotFound, maxValue<'T>), 
                                 isIndexed=true, useThreads=true)     
    ...
    static member FindLastAxis (value: 'T, trgt: DataAndLayout<int64>, src1: DataAndLayout<'T>) =
        ...
        ScalarOps.ApplyAxisFold (op, id, trgt, src1, initial=NotFound, 
                                 isIndexed=true, useThreads=true)             
        ...
        while trgtPosIter.Active do                      
            let maskVal =
                ...
                |> Array.fold (fun s (m, p) -> match m with 
                                               | Some m -> s && m.Span.[m.FastLayout.UncheckedAddr [|p|]]
                                               | _ -> s) true 
            if maskVal then
                trgtSpan.[trgtPosIter.Addr] <- srcSpan.[srcPosIter.Addr]
                trgtPosIter.MoveNext()
            srcPosIter.MoveNext()
    ...
    static member inline TrueIndices (trgt: DataAndLayout<int64>, src: DataAndLayout<bool>) =
        ...
        let mutable srcPosIter = PosIter32 src.FastLayout
        while trgtPosIter.Active do                      
            if srcSpan.[srcPosIter.Addr] then
                for d in 0 .. src.FastLayout.NDims-1 do
                    trgtSpan.[trgtPosIter.Addr] <- int64 srcPosIter.Pos.[d]
                    trgtPosIter.MoveNext()
            srcPosIter.MoveNext()
