            ...
            for call in calls do
                ...
                | TextureCreate tex ->
                    ...
                    let devAry, resDsc =
                        ...
                        | 1 ->
                            if (TensorManikin.stride tex.Contents).[0] <> 1L then
                            ...
                            devAry.CopyFromDeviceToArray1D 
                                (devVar.DevicePointer, SizeT (nElems * sizeof64<single>), 
                                 SizeT (TensorManikin.offset tex.Contents * sizeof64<single>))
                            (devAry :> System.IDisposable), CudaResourceDesc (devAry)
                        | 2 ->
                            if (TensorManikin.stride tex.Contents).[1] <> 1L then
                            ...
                            use pdv = 
                                new CudaPitchedDeviceVariable<single> 
                                    (devVar.DevicePointer + SizeT (TensorManikin.offset tex.Contents * sizeof64<single>), 
                                     SizeT tex.Contents.Shape.[1], SizeT tex.Contents.Shape.[0], 
                                     SizeT (((TensorManikin.stride tex.Contents).[0]) * sizeof64<single>)) 
                            devAry.CopyFromDeviceToThis (pdv)
                            (devAry :> System.IDisposable), CudaResourceDesc (devAry)
                        | 3 ->
                            if (TensorManikin.stride tex.Contents).[2] <> 1L then
                                failwith "texture contents must be continuous in last dimension"
                            if (TensorManikin.stride tex.Contents).[0] <> 
                               (TensorManikin.stride tex.Contents).[1] * tex.Contents.Shape.[1] then
                            ...
                            devAry.CopyFromDeviceToThis 
                                (devVar.DevicePointer + SizeT (TensorManikin.offset tex.Contents * sizeof64<single>),
                                 ...
                                 SizeT (((TensorManikin.stride tex.Contents).[1]) * sizeof64<single>))
                            (devAry :> System.IDisposable), CudaResourceDesc (devAry)
